...
void cpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    for (int h = 0; h < height; h++)
    {
        ...
        //printf("h %d, offset_out %d, ,offset %d\n", h, offset_out, offset);
        
        for (int w = 0; w < width; w++)
        {
            float *pixel = &image[offset + w * 3];

            //printf("posicion de columna %d, fila actual %d\n", w, h);
...
__global__ void gpu_grayscale(int width, int height, float *image, float *image_out)
{
    ...
    // esta parte es la misma que el hello2d.cu que hicimos en clase.
    // entiendo que no tiene sentido hacerlo con shared memory ya que estamos trayendo
    // un una fila y no una matrix asi que no habria beneficio de performance

    int x = threadIdx.x + blockDim.x * blockIdx.x; //indice de columna
    int y = threadIdx.y + blockDim.y * blockIdx.y; //indice de fila

    if (x < width && y < height){

        int pixel = y * width + x; //fila por anbcho de columna mas el offset de la columna.
        int color_pixel = pixel * 3; //vector de 3 colores de cada pixel

        float blue = image[color_pixel + 0];
        float green = image[color_pixel + 1];
        float red = image[color_pixel + 2];

        float gray = blue * 0.0722f + green * 0.7152f + red * 0.2126f;

        image_out[pixel] = gray;
    } 
}
...
__global__ void gpu_gaussian(int width, int height, float *image, float *image_out)
{
    ...
    // Coordenadas locales en el bloque compartido (con padding de 1 alrededor)
    int lx = threadIdx.x + 1;
    int ly = threadIdx.y + 1;

    // Índices planos
    int global_idx = y * width + x;
    int shared_idx = ly * BLOCK_SIZE_SH + lx;

    // Centro
    if (x < width && y < height)
        sh_block[shared_idx] = image[global_idx];
    else
        sh_block[shared_idx] = 0.0f;

    // Lados del halo
    if (threadIdx.x == 0 && x > 0)
        sh_block[ly * BLOCK_SIZE_SH + (lx - 1)] = image[y * width + (x - 1)];

    if (threadIdx.x == BLOCK_SIZE - 1 && x < width - 1)
        sh_block[ly * BLOCK_SIZE_SH + (lx + 1)] = image[y * width + (x + 1)];

    if (threadIdx.y == 0 && y > 0)
        sh_block[(ly - 1) * BLOCK_SIZE_SH + lx] = image[(y - 1) * width + x];
    ...
    if (threadIdx.x == 0 && threadIdx.y == 0 && x > 0 && y > 0)
        sh_block[(ly - 1) * BLOCK_SIZE_SH + (lx - 1)] = image[(y - 1) * width + (x - 1)];
    ...
    // Aplicar el filtro (solo si no estás en borde de la imagen)
    if (x > 0 && x < width - 1 && y > 0 && y < height - 1)
    {
        ...
        image_out[global_idx] = gpu_applyFilter(&sh_block[(ly - 1) * BLOCK_SIZE_SH + (lx - 1)],
...
__global__ void gpu_sobel(int width, int height, float *image, float *image_out)
{
    ...
    //copiar cpu sobel y modificarlo para que se acomode a la gpu
    //__shared__ float sh_block[BLOCK_SIZE_SH * BLOCK_SIZE_SH];
    ...
    __shared__ float sh_block[BLOCK_SIZE_SH * BLOCK_SIZE_SH];
        ...
        // Aplicar Sobel solo si está dentro del rango válido
        if (x > 0 && x < width - 1 && y > 0 && y < height - 1)
        {
            float sobel_x[9] = {
                1.0f,  0.0f, -1.0f,
            ...
            int sh_offset = ly * BLOCK_SIZE_SH + lx;

            float gx = gpu_applyFilter(&sh_block[sh_offset - BLOCK_SIZE_SH - 1], BLOCK_SIZE_SH, sobel_x, 3);
            float gy = gpu_applyFilter(&sh_block[sh_offset - BLOCK_SIZE_SH - 1], BLOCK_SIZE_SH, sobel_y, 3);

            image_out[global_idx] = sqrtf(gx * gx + gy * gy);
        }
}
